...
export const registerUser = async (req, res, next) => {
  try {
    ...
    // secure true on Render; for local dev set env COOKIE_SECURE=false
    const secure = process.env.COOKIE_SECURE !== "false";
    setSessionCookies(res, session, { secure });

    res.status(201).json(user); // toJSON removes password
  } catch (err) {
    next(err);
  }
};
...
export const loginUser = async (req, res, next) => {
  try {
    ...
    // remove existing sessions for this user (requirement: delete old session)
